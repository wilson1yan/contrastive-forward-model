...
def worker(gpu_id, max_per_gpu, exps):
    ...
    processes = []
    for exp in exps:
        exp['name'] = 'joint_dynamics_' + exp['name']
        args = construct_run_command('cfm/baselines/train_joint_dynamics.py', exp)
        print('Running', args)
...
if __name__ == '__main__':
    ...
    exps = [dict(root=folders, z_dim=[get_z(f) for f in folders]),
            dict(trans_type=['reparam_w_tanh'], batch_size=[128], lr=[1e-3], epochs=[10]),
            dict(seed=[0, 1, 2, 3])]
